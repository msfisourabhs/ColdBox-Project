...
<cfcomponent hint = "validation for fields" >
	...
	<cfargument type = "string" required = "false" name = "field" hint = "field value">	
		...
		<cfquery datasource="CBoxDS" result="fetchEmail">
			SELECT EmailAddress
				FROM Users.Details
			WHERE EmailAddress = <cfqueryparam value=#ARGUMENTS.fieldValue# cfsqltype="cf_sql_varchar" maxlength="100" null="false">
		</cfquery>
		<cfif fetchEmail.RecordCount NEQ 0>
			<cfset generateErrors(ARGUMENTS.field&"="&getModifiedFieldValue()  , "Email address is already registered")>
		</cfif>
		<cfreturn VARIABLES.sChangeAndError>
	</cffunction>
